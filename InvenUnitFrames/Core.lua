...
function IUF:OnClick(button)
	if button == "RightButton" then
		-- nothing
	elseif InterfaceOptionsFrame:IsShown() and InterfaceOptionsFramePanelContainer.displayedPanel == IUF.optionFrame then
...
function IUF:RefreshCamera(model)
	--print(model:GetModelFileID())
	model:SetCamera(0)
	--model:SetCamera(type(model:GetModel()) == "string" and changeModelCamera[model:GetModel():lower()] or 0)
end
...
function IUF:CheckModel()
	if UnitExists("target") and InvenUnitFrames_Target and InvenUnitFrames_Target.portrait and InvenUnitFrames_Target.portrait:IsVisible() then
		--print(InvenUnitFrames_Target.portrait.model3d:GetModel())
