...
function lib:IsDispelable(unit, index)
	if UnitCanAttack("player", unit) then
		auraType, _, _, _, _, _, auraID = select(5, UnitBuff(unit, index))
...
function lib:DispelHelp(unit, usablefunc)
	if next(lib.help) then
		for i = 1, 40 do
			name, _, _, _, auraType, _, _, _, _, _, spellId = UnitDebuff(unit, i)
			if name then
...
function lib:DispelHarm(unit, usablefunc)
	if lib.harm or lib.tranquilize then
		for i = 1, 40 do
			name, _, _, _, auraType, _, _, _, _, _, auraID = UnitBuff(unit, i)
			if name then
