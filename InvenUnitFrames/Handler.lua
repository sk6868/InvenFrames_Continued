...
local updateEvents = { "OnUpdate", "UNIT_NAME_UPDATE", "UNIT_MAXHEALTH", "UNIT_HEALTH", "UNIT_HEALTH_FREQUENT", "UNIT_DISPLAYPOWER", "RAID_TARGET_UPDATE", "PLAYER_UPDATE_RESTING", "UNIT_SPELLCAST_START", "UNIT_AURA", "UNIT_COMBO_POINTS", "UNIT_HEAL_PREDICTION" }
...
function IUF:RegisterHandlerEvents()
	...
	handlers:RegisterUnitEvent("UNIT_COMBO_POINTS", "player", "vehicle")
...
function IUF:RegisterObjectEvents(object)
	...
	object:RegisterUnitEvent("UNIT_POWER", object.realunit, object.petunit)
...
function handlers:OnUpdate()
	if self.unit and UnitExists(self.unit) and self.realunit and UnitExists(self.realunit) then
		...
		self.values.tapped = not UnitIsTapDenied(self.unit)
...
function handlers:UNIT_DISPLAYPOWER()
	if UnitAlternatePowerInfo(self.unit) and not UnitIsPlayer(self.unit) then
		self.values.powertype = "ALTERNATE"
		self.values.powermin = select(2, UnitAlternatePowerInfo(self.unit)) or 0
		...
		self.values.powermax = UnitPowerMax(self.unit, self.values.powertype)
		self.values.power = UnitPower(self.unit, self.values.powertype)
...
handlers.UNIT_POWER_FREQUENT = handlers.UNIT_POWER
...
function handlers:UNIT_SPELLCAST_START()
	...
	elseif UnitChannelInfo(self.unit) then
		...
		self.values.castingName, self.values.castingRank, _, self.values.castingIcon, self.values.castingStartTime, endTime, _, self.values.castingIsShield = UnitChannelInfo(self.unit)
		self.values.castingEndTime = endTime
	else
