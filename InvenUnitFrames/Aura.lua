...
local function auraUpdate(object)
	...
	if object.buff.num > 0 then
			...
			if object.buff.filters.harmDispel or object.buff.filters.harmOhter then
				for i = 1, 40 do
					if numBuff < object.buff.num then
						auraName, _, auraIcon, auraCount, auraType, auraDuration, auraEnumDebuffTime, auraCaster, _, _, auraSpellID = UnitBuff(object.unit, i)
						if auraName then
		...
		elseif object.buff.filters.helpOhter then
			for i = 1, 40 do
				if numBuff < object.buff.num then
					auraName, _, auraIcon, auraCount, auraType, auraDuration, auraEnumDebuffTime = UnitBuff(object.unit, i)
					if auraName then
						showBuff(object, i, object.buff.filters.helpOhterBig, 0, 0, 0, 0.6)
					else
						break
	...
	if object.debuff.num > 0 then
			...
			if object.debuff.filters.harmMine or object.debuff.filters.harmCast or object.debuff.filters.harmOhter then
				for i = 1, 40 do
					if numDebuff < object.debuff.num then
						auraName, _, auraIcon, auraCount, auraType, auraDuration, auraEnumDebuffTime, auraCaster, _, _, auraSpellID = UnitDebuff(object.unit, i)
						if auraName then
							if playerUnits[auraCaster or ""] then
		...
		elseif isHelp then
			if object.debuff.filters.helpDispel or object.debuff.filters.helpOhter then
				for i = 1, 40 do
					if numDebuff < object.debuff.num then
						auraName, _, auraIcon, auraCount, auraType, auraDuration, auraEnumDebuffTime = UnitDebuff(object.unit, i)
						if auraName then
